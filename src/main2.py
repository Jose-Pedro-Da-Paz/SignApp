...
def gerar_documento():
    ...
    caminho_documento = os.path.join(os.path.dirname(__file__), '..', 'assets', 'CERTIFICADO_VISITACAO.docx')
...
app.iconphoto(False, PhotoImage(file=os.path.join(os.path.dirname(__file__), '..', 'assets', 'logotipo.png')))

# Carregar a imagem da logo
logo_path = os.path.join(os.path.dirname(__file__), '..', 'assets', 'logotipo.png')  # Certifique-se de que sua imagem está no formato GIF
...
# Adicionar Label com o nome do desenvolvedor
Label(app, text="Desenvolvido por José Pedro D.P.F", font=('Times New Roman', 8)).grid(row=13, column=0, columnspan=2, pady=10)
